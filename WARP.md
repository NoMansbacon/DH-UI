...
- Entry point: `src/main.ts` exports `DaggerheartPlugin extends Plugin` with standard Obsidian lifecycle (`onload`, `onunload`).
  ...
  - Markdown processors:
    ...
    - Domain blocks: registers additional processors via helpers from `src/lib/domains/*` (e.g., badges, consumables, trackers, unified rest, damage, vitals). Each registration is wrapped in `try/catch` with console errors on failure.
